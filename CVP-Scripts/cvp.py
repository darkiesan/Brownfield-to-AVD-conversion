...
class Cvp( Jsonable ):
   ...
   def removeImageBundleAppliedToDevice( self, device, imageBundle ):
      ...
      imageBundleInfo = self.cvpService.getImageBundleByName( imageBundle.name )
      imageBundleKey = imageBundleInfo[ 'key' ]
      if not imageBundleKey:
         raise cvpServices.CvpError( errorCodes.INVALID_IMAGE_BUNDLE_NAME )
      return self.cvpService.removeImageBundleAppliedToDevice( device.macAddress,
                                    device.fqdn, imageBundle.name, imageBundleKey )
   ...
   def removeImageBundleAppliedToContainer( self, container, imageBundle ):
      ...
      if imageBundleKey == '':
         raise cvpServices.CvpError( errorCodes.INVALID_IMAGE_BUNDLE_NAME )
      return self.cvpService.removeImageBundleAppliedToContainer( container.name,
   ...
   def bulkImportDevice( self, ipAddressOrNameList, containerName="Undefined",
           executeTask=False, failFast=True ):
      '''
      ...
      taskIdMap, _, ipAddressOrNameList = self.cvpService.bulkAddToInventory(
              deviceIpToContainerNameMap, failFast )
      if not ipAddressOrNameList:
         return {}, {}
   ...
   def generateConfigletForDevice( self, device, configletBuilder, inputs=None ):
      ...
      for genConfiglet in genConfigMapInfo[ 'generatedConfigletMappers' ]:
         if configletInfo[ 'key' ] == genConfiglet[ 'configletId' ] and\
            genConfiglet[ 'containerId' ]:
            containerInfo = self.cvpService.getContainerInfoByKey(
                                          genConfiglet[ 'containerId' ])
            containerName = containerInfo[ 'name' ]
   ...
   def monitorTaskStatus( self, taskList, status=Task.COMPLETED, timeout=1800 ):
   ...
   def updateUser(self, user, password=""):
      ...
      self.cvpService.updateUser( user.userId, user.email, password, user.roleList,
                               user.firstName, user.lastName, user.userStatus,
                               user.contactNumber)

   def enableDisableUsers( self, state, users ):
      '''enables/disables the specified users from cvp.'''
      assert all ( isinstance( user, User ) for user in users )
      self.cvpService.enableDisableUsers(state,[ user.userId for user in users ] )
