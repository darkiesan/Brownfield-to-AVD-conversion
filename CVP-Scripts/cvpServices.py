# Copyright (c) 2015-2020 Arista Networks, Inc.  All rights reserved.
# Arista Networks, Inc. Confidential and Proprietary.
'''
@Copyright: 2015-2020 Arista Networks, Inc.
Arista Networks, Inc. Confidential and Proprietary.
...
ONBOARD_DEVICE_BATCH_SIZE = 400
DEVICE_MOVE_BATCH_SIZE = 400
PROVISION_STREAMING_DEVICE_BATCH_SIZE = 400
...
class CvpService( object ):
   ...
   def __init__( self, host, ssl, port, tmpDir='' ):
      if os.environ.get( 'CURRENT_NODE_IP' ) and host in [ 'localhost', '127.0.0.1' ]:
         ...
         host = os.environ.get( 'CURRENT_NODE_IP' )
      self.host = host
      self.ssl = ssl
      self.port = port
      self.tmpDir = tmpDir
   ...
   def downloadImage( self, imageName, imageId, filePath='' ):
      ...
      kwargs[ 'cookies' ] = self.cookies
      kwargs[ 'verify' ] = False
      resp = requests.get( '%s/cvpservice/image/getImagebyId/%s' %
                                              ( self.url_, imageId ),
                                              stream=True, **kwargs )
   ...
   # "provisioned" parameter to this function determines whether to look for only provisioned
   # devices in inventory or also the non-provisioned ones
   # "streaming", if true, means that only consider the entries that are actively streaming
   # "serialNumber" indicates whether to return mac address or serial number in the return map
   def waitForDevicesToBeInInventory( self, ipAddressOrNameList, timeout=360, provisioned=True,
                                      streaming=False, serialNumber=False ):
      ...
      while time.time() <= endTime:
         ...
         for device in devicesInfo:
            if streaming and device['streamingStatus'] != "active":
               continue
            if serialNumber:
               deviceMacOrSN = device['serialNumber']
            else:
               deviceMacOrSN = device['systemMacAddress']
            ...
            if device['ipAddress'] in ipAddressOrNameList:
               found[ device['ipAddress'] ] = deviceMacOrSN
            elif hostname in ipAddressOrNameList:
               found[ hostname ] = deviceMacOrSN
            elif device['fqdn'] in ipAddressOrNameList:
               found[ device['fqdn'] ] = deviceMacOrSN
   ...
   def addToInventory( self, deviceIpAddress, parentContainerId ):
      ...
      # If the device is already in the container then there is nothing more to do
      devicesInContainer = self.getDevicesInContainer( parentContainerId, parentContainerName )
      if deviceIDMap[ deviceIpAddress ] in [ dev['netElementKey'] for dev in devicesInContainer ]:
         return None
   ...
   def onboardDevices( self, deviceIpAddressesOrHostnames, useBatching=True ):
      ...
      while start < end:
         ...
         if len( resp.get( 'hostToDevIdMap', {} ) ) > 0 :
            for k, v in resp[ 'hostToDevIdMap' ].items():
               hostToDevIdMap[ k ] = v
      ...
      if len( hostToDevIdMap ) > 0 :
         onboardResponse[ 'hostToDevIdMap' ] = hostToDevIdMap


      return onboardResponse

   def bulkAddToInventory( self, deviceToContainerNameMap, failFast=True ):
      ...
      Arguments:
         ...
         failFast -- If False, we won't error out and return if one or more device(s)
                     fails to onboard
      Returns:
         taskIDMap -- Map of IP address or host name to the ID of corresponding
         'device add' task ID ( type: map[string]string )
         failed --- Map of devices to their corresponding onboarding errors
         successfulDevices -- List of devices onboarded successfully
      Raises:
         CvpError -- If parameter data structures are incorrect
      '''
      deviceIpAddresses = deviceToContainerNameMap.keys()
      deviceHostnameToDnsIp = {}
      successfulDevices = []
      # Onboard the devices
      onboardResp = self.onboardDevices(deviceIpAddresses)
      failed = {}
      if not failFast and 'failed' in onboardResp[ 'data' ]:
         failedResp = onboardResp[ 'data' ][ 'failed' ]
         for devErrs in failedResp:
            failed.update( devErrs )
            for dev, err in devErrs.iteritems():
               print( "Onboarding of device %s failed with error %s" % (
                   dev, err ) )
      else:
         assert onboardResp[ "data" ] == "success"
      hostToDevIdMaps = onboardResp.get( 'hostToDevIdMap' )
      # This can be None for cvaas.
      if hostToDevIdMaps is not None and len(hostToDevIdMaps) > 0:
         devicesInfo, _ = self.getInventory( populateParentContainerKeyMap=False )
         for device in devicesInfo:
            for dnsIp, devId in onboardResp[ 'hostToDevIdMap' ].items():
               if device[ 'serialNumber' ] == devId:
                  deviceHostnameToDnsIp[ device[ 'hostname' ] ] =  dnsIp
                  successfulDevices.append( device['hostname'] )
      if failed:
         deviceIpAddresses = list( set( deviceIpAddresses ) - set( failed.keys() ) )
         if not deviceIpAddresses:
            return {}, failed, successfulDevices
      # Wait for device to appear in inventory
      deviceIPToMacMap = self.waitForDevicesToBeInInventory( successfulDevices )
      deviceMacToContainerNameMap = {}
      deviceMacToIPMap = {}
      for deviceIP, deviceMac in deviceIPToMacMap.iteritems():
         deviceMacToIPMap[ deviceMac ] = deviceIP
         containerName = deviceToContainerNameMap[ deviceHostnameToDnsIp[ deviceIP ] ]
      ...
      if not deviceMacToContainerNameMap:
         return taskIDMap, failed, successfulDevices
   ...
   def removeConfigletFromContainer( self, containerName, containerKey,
      ...
      for rmCbk in rmCbkl:
         ...
         self.doRequest( requests.post, '%s/cvpservice/configlet/addTempGeneratedConfiglet.do'\
            % self.url_, data=json.dumps( addTempGC ) )
   ...
   def removeImageBundleAppliedToDevice( self, deviceKey, deviceFqdn,
                                             imageBundleName, imageBundleKey):
      '''Removes image bundle applied to the device.
      Arguments:
         deviceKey -- mac address of device (type: string)
         deviceFqdn -- Fully qualified domain name for device (type: string)
         imageBundleName -- name of image bundle (type: string)
      ...
      '''
      data = { 'data': [
               { "info" : "Image Bundle Removal: from  device " + deviceFqdn,
                 "infoPreview" : "<b>Image Bundle Removal:</b> from device  "
                    + deviceFqdn,
                 "action" : "associate",
                 "nodeType" : "imagebundle",
                 "nodeId" : "",
   ...
   def getLogsById( self, taskId ):
      ...
      '''
      taskInfo = self.getTaskById( taskId )
      ccId = taskInfo[ 'ccIdV2' ]
      stageId = taskInfo[ 'stageId' ]
      return self.getAuditLogsById( ccId, stageId=stageId )
   ...
   def updateUser( self, userId, email, password, roleList, firstName, lastName,
                userStatus, contactNumber):
      '''Updates the entities of a user '''
      data = {
               "user": {
               "userStatus": userStatus,
               "userId": userId,
      ...
      self.doRequest( requests.post,"%s/cvpservice/user/updateUser.do?userId=%s"
            % (self.url_, userId),data=json.dumps( data ) )

   def enableDisableUsers( self, state, userNames ):
      '''Enable/Disable users in the system. state is enable/disable
       'userNames' is a list of names.'''
      return self.doRequest( requests.post, "%s/cvpservice/user/enableDisableUsers.do?state=%s"
            % (self.url_,state), data=json.dumps( userNames ) )
...
class CvpCloudService( CvpService ):
   ...
   def getDefaultUsers( self ):
      ...
      return [ 'cvpadmin', 'admin', 'arista-support' ]
   ...
   def authenticate( self, username, password ):
      ...
      authData='{"org":"%s","name":"%s","password":"%s"}' % ( self.tenant,
            username, password )
      authUrl = '%s/api/v1/oauth?provider=local&next=false' % self.url_
   ...
   def addEnrollmentToken( self, reenrollDevices=None ):
      '''Generate device enrollment token'''
      if reenrollDevices is None:
         reenrollDevices = []
      url = '%s/api/v3/services/admin.Enrollment/AddEnrollmentToken' % self.url_
      data = { 'enrollmentToken':
                  {'groups':[],
                   'validFor': '2h',
                   'reenrollDevices': reenrollDevices } }

      resp =  self.doRequest( requests.post, url, data=json.dumps( data ) )

      return resp[0]['enrollmentToken']['token']
   ...
   def onboardDevices( self, deviceIpAddressesOrHostnames, useBatching=True ):
      ...
      Arguments:
         deviceIpAddresses -- List of device IP addresses or host names ( type : list of string )
         useBatching -- Use batching while provisioning ( type : boolean )
      Returns:
         response body
      '''
      # For CVaaS, the device needs to be streaming before it can be provisioned.
      # So, wait for the device to be streaming.
      devicesInfo = super( CvpCloudService,
                           self ).waitForDevicesToBeInInventory( deviceIpAddressesOrHostnames,
                                                                 provisioned=False,
                                                                 streaming=True,
                                                                 serialNumber=True )

      deviceList = []
      for device in deviceIpAddressesOrHostnames:
         deviceList.append( devicesInfo[ device ] )

      resp = super( CvpCloudService, self ).provisionStreamingDevices( deviceList,
                                                         useBatching=useBatching )
      resp[ 'hostToDevIdMap' ] = devicesInfo
      return resp
   ...
   def generateClientCertificateAndPrivateKey( self, org, datasetType, datasetName ):
      ''' Generate client certificate and private key for CvpCloudService

      Arguments:
         org -- Name of tenant that require the certs (type: string)
         datasetType -- "user"
         datasetName -- "admin"
      Returns:
         Absolute path to certificate and key
      '''
      pid = str(os.getpid())
      keyName = "private-" + self.host + "-" + pid + ".pem"
      csrName = "csr-" + self.host + "-" + pid + ".pem"
      certName = org + "-" + self.host + "-" + pid + ".crt"

      # Create Certificate Signing Request (CSR) and Private Key using openssl
      cmd = subprocess.Popen(["openssl",
         "req",
         "-out",
         csrName,
         "-new",
      ...
      _ , _ = cmd.communicate(input=b"IN\nKA\nBanglore\nArista\n\n\n\ncvpadmin\n\n")

      if cmd.returncode != 0:
         raise CvpError(cmd.returncode, "Error in credentials generation")

      # Obtain cert by signing CSR from CA (here admin.<clustername>.corp.arista.io>)
      ca_url = "https://admin." + self.host + ".corp.arista.io/api/v1/cert/client"
      data = {
               "org": org,
      ...
      with open(csrName, 'r') as f:
         csrContents = f.read()
      base64Csr = base64.b64encode(bytes(csrContents))
      data["csrFile"] = base64Csr

      response = requests.post(ca_url, json=data)

      if response.status_code != 200:
         raise CvpError(response.status_code, "Error in getting crt file from server")

      with open(certName, 'w') as f:
         f.write(response.json()['cert'])

      # Return path to certificate and key
      return os.path.abspath(certName), os.path.abspath(keyName)
